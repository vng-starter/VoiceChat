...
public class AutocorrellatedVoiceActivityDetector {
    ...
    private double threshold = 65.0d;
    ...
    public byte[] removeSilence(byte[] voiceSample, float sampleRate) {
        ...
        byte[] correllation = new byte[windowSize];
        byte[] window = new byte[windowSize];
        
        //byte[] temp = Speaker.toByteArray(voiceSample);
...
//        System.out.println((int)((double)silenceCounter / result.length * 100.0d) + "% removed");
        ...
        if (silenceCounter > 0) {
            ...
            byte[] shortenedVoiceSample = new byte[voiceSample.length - silenceCounter];
            int copyCounter = 0;
     ...
     */
    private void applyFadeInFadeOut(byte[] voiceSample, int fadeLength, int startIndex, int endIndex) {
     ...
     */
    private double bruteForceAutocorrelation(byte[] voiceSample, byte[] correllation) {
        
        Arrays.fill(correllation, (byte)0);
